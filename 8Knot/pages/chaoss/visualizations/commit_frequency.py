...
from queries.commits_query import commits_query as rq
import io
...
VIZ_ID = "commit-freq-viz"  # UNIQUE IDENTIFIER FOR VIZUALIZATION
            ...
            [
                ...
                dbc.Popover(
                    [
                        ...
                        dbc.PopoverBody("A graph showing the commits made for a given repo over the course of the past year, allowing maintainers to compare and monitor commit cadence and frequency. Frequent commits can help show the rate of progress in a project."),
                    ],
...
def COMMIT_FREQUENCY_graph(repolist):
...
def process_data(df: pd.DataFrame):
    ...
    df = df[df.author_timestamp >= (dt.date.today() - dt.timedelta(days=365))]
...
def create_figure(df: pd.DataFrame):
    ...
    fig.update_traces(
        hovertemplate="Repo: %{y} <br>Commit Published: %{x} <extra></extra>",
        marker={'size': 10}
